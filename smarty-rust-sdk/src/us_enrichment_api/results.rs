...
pub trait EnrichmentResponse: Clone {
    fn build_url(url: &mut Url, lookup: &mut EnrichmentLookup<Self>) -> Result<(), SDKError>;
}
...
impl EnrichmentResponse for PrincipalResponse {
    fn build_url(url: &mut Url, lookup: &mut EnrichmentLookup<Self>) -> Result<(), SDKError> {
        match url.join(&format!("/lookup/{}", lookup.smarty_key.to_string())) {
            Ok(value) => *url = value,
            Err(err) => {
                return Err(SDKError {
                    code: None,
...
pub struct PrincipalAttributes {
    ...
    pub acres: String,
    pub address_info_privacy: String,
    ...
    #[serde(rename = "deed_ document_page")]
